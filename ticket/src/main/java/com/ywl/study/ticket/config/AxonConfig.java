...
public class AxonConfig {
    ...
    public Binding ticketQueueBinding(){
        return BindingBuilder.bind(ticketQueue()).to(exchange()).with("com.ywl.study.ticket.event.#").noargs();
    }
    /**
     * 当OrderPaidEvent发送到amqp中的时候，可以将该event发送到user队列和saga队列中，
     * 这样一个消息就可同时发送到多个队列中
    ...
    public Binding sagaQueueBinding(){
        return BindingBuilder.bind(ticketSagaQueue()).to(exchange()).with("com.ywl.study.ticket.event.saga.#").noargs();
    }
     ...
     */
    @Autowired
    public void configure(EventHandlingConfiguration ehConfig,SpringAMQPMessageSource ticketMessageSource){
